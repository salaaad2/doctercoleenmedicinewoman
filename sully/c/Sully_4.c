...
#define TEXT "#include <fcntl.h> %c#include <unistd.h> %c#include <stdio.h> %c#include <stdlib.h> %c#define NEXT %cSully_%d.c%c%c #define TEXT %c%s%c%c \
...
close(fd); \
sprintf(buf, %cclang ./%s -o %.7s ; \
./%.7ss%c, name, name,name); \
system(buf); \
return 0; \
}%c"
int main(void) {     char name[96];
